...
Cylinder::Cylinder(const libconfig::Setting &settings) {
  if (!settings.lookupValue("posX", this->pos_.x) ||
      !settings.lookupValue("posY", this->pos_.y) ||
...
HitRecord Cylinder::hitsCapOnly(const Ray &ray) const {
    ...
    if (fabs(ray.dir.y) > EPSILON) {
        ...
        if (t > EPSILON && t < t_min) {
            Math::Vector3D p = ray.pos + ray.dir * t;
            double distance = (p - Math::Vector3D(pos_.x, pos_.y + height_, pos_.z)).length();
    ...
    if (t_min < INFINITY)
        return HitRecord(t_min, ray, *this, normal);
