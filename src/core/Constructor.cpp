  ...
  try {
    ...
    if (camera.has_value()) {
      throw ParsingException(
          "camera object cannot be declared twice. redeclared in file " + file +
          ", ignored.");
    }
...
void RaytracerCore::initMaterials(
  ...
  try {
    ...
    throw ParsingException("material \"" + materialType + "\" does not exist");
  } catch (const LoaderException &exc) {
    throw ParsingException("error loading plugin: " + std::string(exc.what()));
  }
}
...
void RaytracerCore::initPlugins(const std::string &file,
  ...
  try {
    ...
    for (auto i = objects.begin(); i != objects.end(); i++) {
      ...
      for (auto it = i->begin(); it != i->end(); it++) {
        try {
          ...
          if (!it->lookupValue("name", name)) {
            throw ParsingException("missing or invalid \"name\" field");
          }
          if (type == "shape") {
      ...
      scene.bvh = std::make_unique<RayTracer::BVHNode>(scene.shapes_, 0,
                                                       scene.shapes_.size());
...
void RaytracerCore::startThreads(size_t nbThreads) {
  ...
  size_t nbPixelPerThread =
      (this->compressedXResolution_ * this->compressedYResolution_) / nbThreads;
...
void RaytracerCore::loadFile(std::string file,
                             std::optional<RayTracer::Camera> &camera) {
  ...
  try {
    ...
    std::cerr << "error parsing file \"" << file << "\", failed to open file"
              << std::endl;
...
void RaytracerCore::setCamera(RayTracer::Camera &&cam) {
  ...
  cameras_[currentCameraId_]->screen_.bottomSide.x =
      cameras_[currentCameraId_]->screen_.leftSide.y * (double)xResolution_ /
      yResolution_;
  cam1->screen_.pos = {cam1->pos_.x - cam1->screen_.bottomSide.x / 2,
                       cam1->pos_.y - cam1->screen_.leftSide.y / 2,
                       cam1->pos_.z - 1.5};
}
...
RaytracerCore::RaytracerCore(const ArgManager::ArgumentStruct &args)
    : graphic_(args.graphicMode),
      ...
      compressedImage_(
        ...
        {
            this->finalNbImages_ = args.finalNbImages;
